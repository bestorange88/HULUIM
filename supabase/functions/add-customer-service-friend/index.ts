...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Find customer service account
    const { data: customerService, error: csError } = await supabaseAdmin
      .from('profiles')
      .select('id')
      .eq('username', 'customer_service')
      .maybeSingle()

    if (csError || !customerService) {
      return new Response(
        JSON.stringify({ error: '客服账号不存在，请先创建客服账号' }),
